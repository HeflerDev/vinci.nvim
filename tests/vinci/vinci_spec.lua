...
local vinci = require("vinci")
local default = vinci.config
...
describe("tests", function()
  it("works with default values", function()
    vinci.setup()
    assert.are.same(vinci.config, default)
  end)
  ...
  it("works with config overrides", function()
    ...
    vinci.setup({ undercurl = false, underline = false })
    assert.are.same(vinci.config, expected)
  end)
  ...
  it("does not set terminal colors when terminal_colors is false", function()
    clear_term_colors()
    vinci.setup({ terminal_colors = false })
    vinci.load()
  ...
  it("sets terminal colors when terminal_colors is true", function()
    clear_term_colors()
    vinci.setup({ terminal_colors = true })
    vinci.load()
